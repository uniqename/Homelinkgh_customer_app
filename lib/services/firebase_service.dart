...
class FirebaseService {
  ...
  // REVIEWS COLLECTION - Rating & Review System

  /// Create a new review in Firebase
  Future<String> createReview(review) async {
    try {
      // Import review model at top of file when compiling
      ...
      final firestoreData = {
        'fields': {
          ...
          'bookingId': {'stringValue': reviewData['bookingId']},
          'customerId': {'stringValue': reviewData['customerId']},
          'customerName': {'stringValue': reviewData['customerName']},
          'providerId': {'stringValue': reviewData['providerId']},
          'rating': {'doubleValue': reviewData['rating']},
          'reviewText': {'stringValue': reviewData['reviewText']},
          'jobTitle': {'stringValue': reviewData['jobTitle']},
          'createdAt': {'timestampValue': reviewData['createdAt']},
          'isVerifiedCustomer': {'booleanValue': reviewData['isVerifiedCustomer']},
          'jobAmount': {'doubleValue': reviewData['jobAmount']},
          'photoUrls': {
            'arrayValue': {
              'values': (reviewData['photoUrls'] as List).map((url) => {'stringValue': url}).toList()
            }
          },
          if (reviewData['providerResponse'] != null)
            'providerResponse': {'stringValue': reviewData['providerResponse']},
          if (reviewData['respondedAt'] != null)
            'respondedAt': {'timestampValue': reviewData['respondedAt']},
          if (reviewData['categoryRatings'] != null)
            'categoryRatings': {
              'mapValue': {
                'fields': (reviewData['categoryRatings'] as Map<String, dynamic>).map(
                  (key, value) => MapEntry(key, {'doubleValue': value})
                )
              }
            },
          'updatedAt': {'timestampValue': reviewData['updatedAt']},
        }
      };
  ...
  Future<List<dynamic>> queryReviews(Map<String, dynamic> filters) async {
    try {
      // Import Review model at top when compiling
      final providerId = filters['providerId'];
      final customerId = filters['customerId'];
      final bookingId = filters['bookingId'];
      final limit = filters['limit'] ?? 20;

      // Build query URL
      String queryUrl = '$_baseUrl/reviews';

      // Add filters
      if (providerId != null) {
        queryUrl += '?orderBy=providerId&equalTo="$providerId"';
      } else if (customerId != null) {
        queryUrl += '?orderBy=customerId&equalTo="$customerId"';
      } else if (bookingId != null) {
        queryUrl += '?orderBy=bookingId&equalTo="$bookingId"';
      }
      ...
      if (response.statusCode == 200) {
        ...
        final reviews = <dynamic>[];
        for (final doc in documents.take(limit)) {
          final fields = doc['fields'] as Map<String, dynamic>;
          reviews.add(_parseReviewFromFirestore(fields));
        }
  ...
  Future<dynamic> getReview(String reviewId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return _parseReviewFromFirestore(fields);
      }
  ...
  Future<void> updateReview(String reviewId, Map<String, dynamic> updates) async {
    try {
      ...
      updates.forEach((key, value) {
        if (value is String) {
          updateFields[key] = {'stringValue': value};
        } else if (value is double) {
          updateFields[key] = {'doubleValue': value};
        } else if (value is bool) {
          updateFields[key] = {'booleanValue': value};
        }
      });

      final updateData = {'fields': updateFields};
  ...
  Future<dynamic> getProviderRatingStats(String providerId) async {
    try {
      final response = await http.get(
        Uri.parse('$_baseUrl/provider_rating_stats/$providerId'),
        headers: _headers,
      );
      ...
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        final fields = data['fields'] as Map<String, dynamic>;
        return _parseProviderRatingStatsFromFirestore(fields);
      }
  ...
  Future<void> updateProviderRatingStats(String providerId, Map<String, dynamic> stats) async {
    try {
      final firestoreData = {
        'fields': {
          'providerId': {'stringValue': stats['providerId']},
          'overallRating': {'doubleValue': stats['overallRating']},
          'totalReviews': {'integerValue': stats['totalReviews'].toString()},
          'ratingDistribution': {
            'mapValue': {
              'fields': (stats['ratingDistribution'] as Map<dynamic, dynamic>).map(
                (key, value) => MapEntry(key.toString(), {'integerValue': value.toString()})
              )
            }
          },
          'categoryAverages': {
            'mapValue': {
              'fields': (stats['categoryAverages'] as Map<String, dynamic>).map(
                (key, value) => MapEntry(key, {'doubleValue': value})
              )
            }
          },
          'recentReviews': {
            'arrayValue': {
              'values': (stats['recentReviews'] as List).map((id) => {'stringValue': id}).toList()
            }
          },
          'lastUpdated': {'timestampValue': stats['lastUpdated']},
        }
      };
      ...
      print('Error updating provider rating stats: $e');
      rethrow;
    }
  }

  /// Upload review photo to Firebase Storage (stub - needs Firebase Storage setup)
  Future<String> uploadReviewPhoto(String reviewId, file, int photoIndex) async {
    try {
      // In production, this would upload to Firebase Storage
      // For now, return a placeholder URL
      final photoUrl = 'https://storage.googleapis.com/homelinkgh-reviews/$reviewId/photo_$photoIndex.jpg';
      print('Photo upload stub - would upload to: $photoUrl');
      return photoUrl;
    } catch (e) {
  ...
  /// Parse review from Firestore format
  dynamic _parseReviewFromFirestore(Map<String, dynamic> fields) {
    // This returns a Map that can be converted to Review model
    return {
      'id': fields['id']?['stringValue'] ?? '',
      'bookingId': fields['bookingId']?['stringValue'] ?? '',
      'customerId': fields['customerId']?['stringValue'] ?? '',
      'customerName': fields['customerName']?['stringValue'] ?? '',
      'providerId': fields['providerId']?['stringValue'] ?? '',
      'rating': fields['rating']?['doubleValue'] ?? 0.0,
      'reviewText': fields['reviewText']?['stringValue'] ?? '',
      'jobTitle': fields['jobTitle']?['stringValue'] ?? '',
      'createdAt': fields['createdAt']?['timestampValue'] ?? DateTime.now().toIso8601String(),
      'isVerifiedCustomer': fields['isVerifiedCustomer']?['booleanValue'] ?? true,
      'jobAmount': fields['jobAmount']?['doubleValue'] ?? 0.0,
      'photoUrls': fields['photoUrls']?['arrayValue']?['values']
          ?.map((v) => v['stringValue'] as String)
          ?.toList() ?? [],
      'providerResponse': fields['providerResponse']?['stringValue'],
      'respondedAt': fields['respondedAt']?['timestampValue'],
      'categoryRatings': fields['categoryRatings']?['mapValue']?['fields']
          ?.map((k, v) => MapEntry(k, v['doubleValue'] as double)),
    };
  }

  /// Parse provider rating stats from Firestore
  dynamic _parseProviderRatingStatsFromFirestore(Map<String, dynamic> fields) {
    return {
      'providerId': fields['providerId']?['stringValue'] ?? '',
      'overallRating': fields['overallRating']?['doubleValue'] ?? 0.0,
      'totalReviews': int.parse(fields['totalReviews']?['integerValue'] ?? '0'),
      'ratingDistribution': fields['ratingDistribution']?['mapValue']?['fields']
          ?.map((k, v) => MapEntry(int.parse(k), int.parse(v['integerValue'] ?? '0'))) ?? {},
      'categoryAverages': fields['categoryAverages']?['mapValue']?['fields']
          ?.map((k, v) => MapEntry(k, v['doubleValue'] as double)) ?? {},
      'recentReviews': fields['recentReviews']?['arrayValue']?['values']
          ?.map((v) => v['stringValue'] as String)
          ?.toList() ?? [],
      'lastUpdated': fields['lastUpdated']?['timestampValue'] ?? DateTime.now().toIso8601String(),
    };
  }
  ...
  Future<void> createSavedPaymentMethod(SavedPaymentMethod paymentMethod) async {
    try {
      final data = {
        'fields': {
          'id': {'stringValue': paymentMethod.id},
          'userId': {'stringValue': paymentMethod.userId},
          'type': {'stringValue': paymentMethod.type},
          'token': {'stringValue': paymentMethod.token},
          'last4': {'stringValue': paymentMethod.last4},
          'brand': {'stringValue': paymentMethod.brand},
          'expiryMonth': paymentMethod.expiryMonth != null
              ? {'stringValue': paymentMethod.expiryMonth!}
              : {'nullValue': null},
          'expiryYear': paymentMethod.expiryYear != null
              ? {'stringValue': paymentMethod.expiryYear!}
              : {'nullValue': null},
          'phoneNumber': paymentMethod.phoneNumber != null
              ? {'stringValue': paymentMethod.phoneNumber!}
              : {'nullValue': null},
          'cardholderName': paymentMethod.cardholderName != null
              ? {'stringValue': paymentMethod.cardholderName!}
              : {'nullValue': null},
          'isDefault': {'booleanValue': paymentMethod.isDefault},
          'createdAt': {'timestampValue': paymentMethod.createdAt.toUtc().toIso8601String()},
          'lastUsed': {'timestampValue': paymentMethod.lastUsed.toUtc().toIso8601String()},
        }
      };

      await http.post(
        Uri.parse('$_baseUrl/saved_payment_methods'),
        headers: _headers,
        body: json.encode(data),
      );
    } catch (e) {
      print('Error creating saved payment method: $e');
      throw Exception('Failed to create saved payment method');
    }
  }
  ...
  Future<List<SavedPaymentMethod>> getSavedPaymentMethods(String userId) async {
    try {
      final response = await http.get(
        Uri.parse('$_baseUrl/saved_payment_methods?userId=$userId'),
        headers: _headers,
      );
      ...
      if (response.statusCode == 200) {
        ...
        return documents
            .map((doc) => _parseSavedPaymentMethodFromFirestore(doc['fields']))
            .toList();
      }
  ...
  Future<SavedPaymentMethod?> getSavedPaymentMethod(String paymentMethodId) async {
    try {
      final response = await http.get(
        Uri.parse('$_baseUrl/saved_payment_methods/$paymentMethodId'),
        headers: _headers,
      );

      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        return _parseSavedPaymentMethodFromFirestore(data['fields']);
      }

      return null;
    } catch (e) {
      print('Error getting saved payment method: $e');
      return null;
    }
  }

  /// Update a saved payment method
  Future<void> updateSavedPaymentMethod(String paymentMethodId, Map<String, dynamic> updates) async {
    try {
      ...
      print('Error updating saved payment method: $e');
      throw Exception('Failed to update saved payment method');
    }
  }
  ...
  Future<void> deleteSavedPaymentMethod(String paymentMethodId) async {
    try {
      ...
      print('Error deleting saved payment method: $e');
      throw Exception('Failed to delete saved payment method');
    }
  }

  /// Parse saved payment method from Firestore format
  SavedPaymentMethod _parseSavedPaymentMethodFromFirestore(Map<String, dynamic> fields) {
    return SavedPaymentMethod(
      id: fields['id']?['stringValue'] ?? '',
      userId: fields['userId']?['stringValue'] ?? '',
      type: fields['type']?['stringValue'] ?? 'card',
      token: fields['token']?['stringValue'] ?? '',
      last4: fields['last4']?['stringValue'] ?? '',
      brand: fields['brand']?['stringValue'] ?? '',
      expiryMonth: fields['expiryMonth']?['stringValue'],
      expiryYear: fields['expiryYear']?['stringValue'],
      phoneNumber: fields['phoneNumber']?['stringValue'],
      cardholderName: fields['cardholderName']?['stringValue'],
      isDefault: fields['isDefault']?['booleanValue'] ?? false,
      createdAt: DateTime.parse(fields['createdAt']?['timestampValue'] ?? DateTime.now().toIso8601String()),
      lastUsed: DateTime.parse(fields['lastUsed']?['timestampValue'] ?? DateTime.now().toIso8601String()),
    );
  }
  ...
  Future<void> createReferralCode(String userId, String code) async {
    try {
      final data = {
        'fields': {
          'userId': {'stringValue': userId},
      ...
      await http.post(Uri.parse('$_baseUrl/referral_codes'), headers: _headers, body: json.encode(data));
    } catch (e) {
  ...
  Future<String?> getReferralCodeByUserId(String userId) async {
    try {
      final response = await http.get(Uri.parse('$_baseUrl/referral_codes?orderBy=userId&equalTo="$userId"'), headers: _headers);
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        final docs = data['documents'] as List? ?? [];
        if (docs.isNotEmpty) {
          return docs[0]['fields']['code']['stringValue'];
        }
      }
      return null;
    } catch (e) {
  ...
  Future<bool> checkReferralCodeExists(String code) async {
    try {
      final response = await http.get(Uri.parse('$_baseUrl/referral_codes?orderBy=code&equalTo="$code"'), headers: _headers);
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        return (data['documents'] as List?)?.isNotEmpty ?? false;
      }
      return false;
    } catch (e) {
  ...
  Future<String?> getUserIdByReferralCode(String code) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final docs = data['documents'] as List? ?? [];
        if (docs.isNotEmpty) {
          return docs[0]['fields']['userId']['stringValue'];
        }
      }
      return null;
    } catch (e) {
      print('Error getting user ID by code: $e');
      return null;
    }
  }

  /// Create referral
  Future<void> createReferral(referral) async {
    try {
      final referralData = referral.toMap();
      final data = {
        'fields': {
          'id': {'stringValue': referralData['id']},
          'referrerId': {'stringValue': referralData['referrerId']},
          'referredUserId': {'stringValue': referralData['referredUserId']},
          'referralCode': {'stringValue': referralData['referralCode']},
          'createdAt': {'timestampValue': referralData['createdAt']},
          'status': {'stringValue': referralData['status']},
          'referrerReward': {'doubleValue': referralData['referrerReward']},
          'referredReward': {'doubleValue': referralData['referredReward']},
          if (referralData['referredUserName'] != null) 'referredUserName': {'stringValue': referralData['referredUserName']},
          if (referralData['referredUserPhone'] != null) 'referredUserPhone': {'stringValue': referralData['referredUserPhone']},
        }
      };
      await http.post(Uri.parse('$_baseUrl/referrals'), headers: _headers, body: json.encode(data));
    } catch (e) {
      print('Error creating referral: $e');
    }
  }

  /// Get pending referral by user ID
  Future<dynamic> getPendingReferralByUserId(String userId) async {
    try {
      final response = await http.get(Uri.parse('$_baseUrl/referrals?orderBy=referredUserId&equalTo="$userId"'), headers: _headers);
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        final docs = data['documents'] as List? ?? [];
        for (var doc in docs) {
          if (doc['fields']['status']['stringValue'] == 'pending') {
            return _parseReferralFromFirestore(doc['fields']);
          }
        }
      }
      return null;
    } catch (e) {
      print('Error getting pending referral: $e');
      return null;
    }
  }
  ...
  Future<void> updateReferral(String referralId, Map<String, dynamic> updates) async {
    try {
      final updateFields = <String, dynamic>{};
      updates.forEach((key, value) {
        if (value is String) updateFields[key] = {'stringValue': value};
        else if (value is double) updateFields[key] = {'doubleValue': value};
        else if (value is bool) updateFields[key] = {'booleanValue': value};
      });
      await http.patch(Uri.parse('$_baseUrl/referrals/$referralId'), headers: _headers, body: json.encode({'fields': updateFields}));
    } catch (e) {
      print('Error updating referral: $e');
    }
  }

  /// Get referral by ID
  Future<dynamic> getReferral(String referralId) async {
    try {
      final response = await http.get(Uri.parse('$_baseUrl/referrals/$referralId'), headers: _headers);
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        return _parseReferralFromFirestore(data['fields']);
      }
      return null;
    } catch (e) {
      print('Error getting referral: $e');
      return null;
    }
  }

  /// Get referrals by referrer ID
  Future<List<dynamic>> getReferralsByReferrerId(String referrerId) async {
    try {
      final response = await http.get(Uri.parse('$_baseUrl/referrals?orderBy=referrerId&equalTo="$referrerId"'), headers: _headers);
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        final docs = data['documents'] as List? ?? [];
        return docs.map((doc) => _parseReferralFromFirestore(doc['fields'])).toList();
      }
      return [];
    } catch (e) {
  ...
  /// Get referral leaderboard rank
  Future<int> getReferralLeaderboardRank(String userId) async {
    // Stub - would need server-side aggregation
    return 0;
  }

  /// Get referral leaderboard
  Future<List<Map<String, dynamic>>> getReferralLeaderboard({String period = 'all-time', int limit = 50}) async {
    // Stub - would need server-side aggregation
    return [];
  }

  /// Update referral stats
  Future<void> updateReferralStats(String userId, Map<String, dynamic> stats) async {
    try {
      final data = {
        'fields': {
          'userId': {'stringValue': userId},
          'referralCode': {'stringValue': stats['referralCode']},
          'totalReferrals': {'integerValue': stats['totalReferrals'].toString()},
          'completedReferrals': {'integerValue': stats['completedReferrals'].toString()},
          'pendingReferrals': {'integerValue': stats['pendingReferrals'].toString()},
          'earnedRewards': {'doubleValue': stats['earnedRewards']},
          'leaderboardRank': {'integerValue': stats['leaderboardRank'].toString()},
          if (stats['lastReferralDate'] != null) 'lastReferralDate': {'timestampValue': stats['lastReferralDate']},
        }
      };
      await http.patch(Uri.parse('$_baseUrl/referral_stats/$userId'), headers: _headers, body: json.encode(data));
    } catch (e) {
  ...
  Future<dynamic> getReferralByReferredUserId(String userId) async {
    try {
      final response = await http.get(Uri.parse('$_baseUrl/referrals?orderBy=referredUserId&equalTo="$userId"'), headers: _headers);
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        final docs = data['documents'] as List? ?? [];
        if (docs.isNotEmpty) {
          return _parseReferralFromFirestore(docs[0]['fields']);
        }
      }
      return null;
    } catch (e) {
      print('Error getting referral by user: $e');
      return null;
    }
  }
  ...
  Future<Map<String, dynamic>?> getUserInfo(String userId) async {
    try {
      final response = await http.get(Uri.parse('$_baseUrl/users/$userId'), headers: _headers);
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        return {
  ...
  /// Parse referral from Firestore
  dynamic _parseReferralFromFirestore(Map<String, dynamic> fields) {
    return {
      'id': fields['id']?['stringValue'] ?? '',
      'referrerId': fields['referrerId']?['stringValue'] ?? '',
      'referredUserId': fields['referredUserId']?['stringValue'] ?? '',
      'referralCode': fields['referralCode']?['stringValue'] ?? '',
      'createdAt': fields['createdAt']?['timestampValue'] ?? DateTime.now().toIso8601String(),
      'completedAt': fields['completedAt']?['timestampValue'],
      'status': fields['status']?['stringValue'] ?? 'pending',
      'referrerReward': fields['referrerReward']?['doubleValue'] ?? 40.0,
      'referredReward': fields['referredReward']?['doubleValue'] ?? 20.0,
      'referredUserName': fields['referredUserName']?['stringValue'],
      'referredUserPhone': fields['referredUserPhone']?['stringValue'],
      'firstBookingId': fields['firstBookingId']?['stringValue'],
    };
  }
}
