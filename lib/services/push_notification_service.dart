...
class PushNotificationService {
  ...
  Future<void> _initializeFCMToken() async {
    try {
      ...
      if (_fcmToken != null) {
        ...
        // Save token to Supabase so Edge Function can look it up for push delivery
  ...
  /// Save FCM token to Supabase so the Edge Function can deliver background push
  Future<void> _saveTokenToSupabase(String token) async {
    try {
      ...
      // Update user_profiles with current FCM token
      await supabase.supabase
          .from('user_profiles')
          .update({'fcm_token': token, 'updated_at': DateTime.now().toIso8601String()})
          .eq('id', user.id);

      // Also update providers table if the user is a provider
      await supabase.supabase
          .from('providers')
          .update({'fcm_token': token, 'updated_at': DateTime.now().toIso8601String()})
          .eq('user_id', user.id);
    } catch (e) {
      // Non-fatal — token will still work for foreground notifications
      print('⚠️ Could not save FCM token to Supabase: $e');
    }
  }
  ...
  Future<void> _initializeLocalNotifications() async {
    ...
    const AndroidNotificationChannel channel = AndroidNotificationChannel(
      'homelinkgh_high_importance', // id
      'HomeLinkGH Notifications', // name
      description: 'Important notifications from HomeLinkGH',
  ...
  void _setupNotificationHandlers() {
    ...
    // Simulate periodic notification checks for pending notifications
      ...
      await _localNotifications.show(
        ...
        NotificationDetails(
          android: AndroidNotificationDetails(
            'homelinkgh_high_importance',
            'HomeLinkGH Notifications',
            channelDescription: 'Important notifications from HomeLinkGH',
            importance: Importance.high,
  ...
  /// Save notification to history in Firebase
  Future<void> _saveNotificationToHistory(RemoteMessage message) async {
    try {
      ...
      final notificationData = {
        ...
        'category': message.data['category'] ?? 'system_updates',
        'data': message.data,
