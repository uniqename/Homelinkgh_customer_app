...
class VerificationService {
  ...
  // Driver's License verification - only used if Ghana Card is not available
  static Future<Map<String, dynamic>> verifyDriversLicense({
    ...
    try {
      ...
      return {
        ...
        'verificationDate': DateTime.now().toIso8601String(),
        'trustScore': 80, // Lower trust score when Ghana Card is not available
  ...
  static int calculateTrustScore({
    ...
    // Base score for identity verification - Ghana Card is strongly preferred
    if (identityVerified) {
      score += verificationType == 'ghana_card' ? 45 : 30; // Ghana Card gets significantly higher base score
    }
  ...
  // Preferred verification method - always try Ghana Card first
  static Future<Map<String, dynamic>> verifyIdentity({
    ...
    // Always prioritize Ghana Card verification if provided
    if (ghanaCardNumber != null && ghanaCardNumber.isNotEmpty) {
      return await verifyGhanaCard(
        cardNumber: ghanaCardNumber,
        fullName: fullName,
        dateOfBirth: dateOfBirth ?? DateTime.now().subtract(const Duration(days: 365 * 25)),
        cardImage: documentImage,
      );
    }
    
    // Only use driver's license if Ghana Card is not available
    if (driversLicenseNumber != null && driversLicenseNumber.isNotEmpty) {
      return await verifyDriversLicense(
        licenseNumber: driversLicenseNumber,
        fullName: fullName,
        issuingCountry: issuingCountry ?? 'US',
        issuingState: issuingState ?? 'CA',
    ...
    return {
      'success': false,
      'error': 'No valid identification document provided. Ghana Card is preferred.',
      'verified': false,
    };
  }
