...
class _ProviderOnboardingScreenState extends State<ProviderOnboardingScreen> {
  ...
  Future<void> _submitApplication() async {
    ...
    try {
      ...
      if (user == null) throw Exception('You must be signed in to submit an application.');

      // Upload Ghana Card images to Supabase Storage (best-effort)
      String? frontImageUrl;
      String? backImageUrl;
      String? selfieImageUrl;

      try {
        final bytes = await _ghanaCardFrontImage!.readAsBytes();
        frontImageUrl = await supabase.uploadFile('provider-docs', '${user.id}/ghana_card_front.jpg', bytes);
      } catch (e) {
        print('⚠️ Could not upload front image: $e');
      }

      try {
        final bytes = await _ghanaCardBackImage!.readAsBytes();
        backImageUrl = await supabase.uploadFile('provider-docs', '${user.id}/ghana_card_back.jpg', bytes);
      } catch (e) {
        print('⚠️ Could not upload back image: $e');
      }

      if (_selfieImage != null) {
        try {
          final bytes = await _selfieImage!.readAsBytes();
          selfieImageUrl = await supabase.uploadFile('provider-docs', '${user.id}/selfie.jpg', bytes);
        } catch (e) {
          print('⚠️ Could not upload selfie: $e');
        }
      }
      ...
      final providerData = {
        ...
        'ghana_card_number': _ghanaCardNumber,
        'ghana_card_front_url': frontImageUrl,
        'ghana_card_back_url': backImageUrl,
      ...
      // Update the existing providers record created during registration,
      // or insert a new one if it doesn't exist yet.
      final existing = await supabase.supabase
          .from('providers')
          .select('id')
          .eq('user_id', user.id)
          .maybeSingle();
      ...
      if (existing != null) {
        ...
        await supabase.supabase.from('providers').insert(providerData);
      }

      // Also update user profile with the confirmed name and phone
      await supabase.updateUserProfile(user.id, {
        'name': _fullName,
        'phone': _phoneNumber,
      });

      // Persist minimal session data locally
      ...
      await prefs.setBool('provider_verified', false);
