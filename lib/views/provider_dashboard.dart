...
class _ProviderDashboardState extends State<ProviderDashboard> {
  ...
  // Provider's services — loaded from Supabase profile
  List<String> providerServices = [];

  // Real service requests from Supabase (replaces all mock data)
  List<Map<String, dynamic>> _newJobRequests = [];
  StreamSubscription<List<Map<String, dynamic>>>? _requestsSubscription;

  final _quoteService = QuoteService();
  final _supabaseService = SupabaseService();

  // Upcoming confirmed bookings — loaded from Supabase (empty on first load)
  ...
  String _providerDisplayName = 'Provider';
  ...
  Future<void> _loadProviderData() async {
    try {
      ...
      // Load provider profile from Supabase to get their services
      final providers = await _supabaseService.supabase
          .from('providers')
          .select()
          .eq('user_id', user.id)
          .maybeSingle();
      ...
      if (providers != null) {
        services = List<String>.from(providers['services'] ?? []);
        setState(() {
          providerServices = services;
          _providerDisplayName = providers['name'] ?? widget.provider?.name ?? 'Provider';
          isLoading = false;
        });
      } else {
        // Fall back to widget.provider services if no Supabase record yet
        services = widget.provider?.services ?? ['House Cleaning', 'Plumbing', 'Electrical Services'];
        setState(() {
          providerServices = services;
          _providerDisplayName = widget.provider?.name ?? 'Provider';
          isLoading = false;
        });
      }

      // Subscribe to real-time open service requests matching this provider
      _requestsSubscription = _quoteService
          .watchOpenRequestsForProvider(services)
          .listen((rows) {
        if (mounted) {
          setState(() {
            _newJobRequests = rows.map(_mapRequestToCard).toList();
          });
        }
      });
    } catch (e) {
  ...
  /// Map Supabase snake_case fields to the format the job card widgets expect
  Map<String, dynamic> _mapRequestToCard(Map<String, dynamic> row) {
    DateTime createdAt;
    try {
    ...
    try {
      if (row['scheduled_date'] != null) {
        scheduledDate = DateTime.parse(row['scheduled_date'].toString());
      }
    } catch (_) {}

    final urgency = row['urgency']?.toString() ?? '';
    final isUrgent = urgency.toLowerCase().contains('urgent') ||
        urgency.toLowerCase().contains('emergency');

    return {
      'id': row['id']?.toString() ?? '',
      'serviceName': row['service_type']?.toString() ?? 'Service',
      'customerName': row['customer_name']?.toString() ?? 'Customer',
      'customerPhone': row['customer_phone']?.toString() ?? '',
      'location': row['location']?.toString() ?? 'Location not specified',
      'distance': 0.0,
      'urgency': isUrgent ? 'Urgent' : 'Standard',
      'description': row['description']?.toString() ?? '',
      'budget': double.tryParse(row['budget']?.toString() ?? '0') ?? 0.0,
      'timeSlot': scheduledDate != null
          ? '${scheduledDate.day}/${scheduledDate.month}/${scheduledDate.year}'
          : urgency.isNotEmpty
              ? urgency
              : 'Flexible',
      'postedTime': createdAt,
      'images': List<String>.from(row['media_urls'] ?? []),
      'videos': <String>[],
      'additionalNotes': '',
    };
  }
