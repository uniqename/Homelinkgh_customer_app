...
class _AuthScreenState extends State<AuthScreen> with TickerProviderStateMixin {
      ...
      body: SafeArea(
        child: Column(
          children: [
            ...
            // Tab Bar and Content — light Theme override so form fields
            // are readable against the white container (global theme is dark)
            Expanded(
              child: Theme(
  ...
  Future<void> _handleSignIn() async {
    ...
    try {
      ...
      // Persist session locally too
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString('userEmail', _signInEmailController.text.trim());
  ...
  Future<void> _handleRegister() async {
    ...
    try {
      ...
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString('userEmail', _registerEmailController.text.trim());
      await prefs.setString('userName', _registerNameController.text.trim());
      await prefs.setString('userPhone', _registerPhoneController.text.trim());
      await prefs.setString('userType', widget.userType);
      await prefs.setBool('isLoggedIn', true);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Account created! Welcome to HomeLinkGH!'),
  ...
  Future<void> _navigateToHome() async {
    ...
    switch (widget.userType) {
      case 'provider':
        // Check if this provider has already completed onboarding in Supabase
        ...
        if (user != null) {
          ...
          if (existing == null) {
            // First login — create a pending providers record and go to onboarding
            await SupabaseService().supabase.from('providers').insert({
              'user_id': user.id,
              'name': (await SharedPreferences.getInstance()).getString('userName') ?? '',
              'email': (await SharedPreferences.getInstance()).getString('userEmail') ?? '',
              'phone': (await SharedPreferences.getInstance()).getString('userPhone') ?? '',
              'services': <String>[],
              'status': 'pending',
            });
            homeScreen = const ProviderOnboardingScreen();
          } else {
      ...
      default:
        ...
        homeScreen = hasCompletedOnboarding
            ? const UnifiedCustomerHomeScreen()
