...
class AdminVerificationManagementScreen extends StatefulWidget {
  ...
  const AdminVerificationManagementScreen({super.key, required this.adminId});

  @override
  State<AdminVerificationManagementScreen> createState() =>
      _AdminVerificationManagementScreenState();
}

class _AdminVerificationManagementScreenState
    extends State<AdminVerificationManagementScreen> {
  ...
  Widget build(BuildContext context) {
    return AdminAuthGuard(
      screenName: 'Verification Management',
      child: Scaffold(
        appBar: AppBar(
  ...
  Widget _buildVerificationsList() {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final isSelected =
            _selectedVerification?['verification_id'] ==
        ...
        return Container(
          ...
          decoration: BoxDecoration(
            ...
            border: isSelected
                ? Border.all(color: const Color(0xFF2E8B57))
                : null,
          ),
          child: ListTile(
            ...
            subtitle: Column(
              ...
              children: [
                Text(
                  '${verification['user_type']?.toString().toUpperCase() ?? 'USER'}',
                ),
                Text(
              ...
              child: Column(
                ...
                children: [
                  _buildInfoCard('Personal Information', [
                    ...
                    _buildInfoRow(
                      'Ghana Card Number',
                      personalInfo['ghana_card_number'],
                    ),
                    _buildInfoRow(
                      'Date of Birth',
                  ...
                  _buildInfoCard('Verification Status', [
                    ...
                    _buildInfoRow(
                      'Trust Level',
                      verification['trust_level']?.toString().toUpperCase(),
                    ),
                    _buildInfoRow(
                      'Submitted',
                      _formatDate(verification['submission_date']),
                    ),
                    _buildInfoRow(
                      'Last Updated',
                      _formatDate(verification['last_updated']),
                    ),
                  ]),
                  ...
                  _buildInfoCard('Documents', [
                    _buildDocumentRow(
                      'Ghana Card Front',
                      verification['documents']?['front_image_path'],
                    ),
                    _buildDocumentRow(
                      'Ghana Card Back',
                      verification['documents']?['back_image_path'],
                    ),
                    _buildDocumentRow(
                      'Selfie',
                      verification['documents']?['selfie_image_path'],
                    ),
                  ]),
                  ...
                  if (verification['additional_data'] != null)
                    _buildInfoCard('Additional Information', [
                      ...
                      _buildInfoRow(
                        'Email',
                        verification['additional_data']?['email'],
                      ),
                      _buildInfoRow(
                        'Services',
                        verification['additional_data']?['services']?.join(
                          ', ',
                        ),
                      ),
                      _buildInfoRow(
                        'Hourly Rate',
                        'GHS ${verification['additional_data']?['hourly_rate']}',
                      ),
                    ]),

                  const SizedBox(height: 16),

                  if (verification['verification_history'] != null &&
                      verification['verification_history'].isNotEmpty)
  ...
  void _loadPendingVerifications() async {
    ...
    try {
      final verifications =
          await GhanaCardVerificationService.getPendingVerifications();
      setState(() {
  ...
  void _showVerificationStatistics() async {
    final stats =
        await GhanaCardVerificationService.getVerificationStatistics();
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              _buildStatRow(
                'Verification Rate',
                '${stats['verification_rate'].toStringAsFixed(1)}%',
              ),
              const SizedBox(height: 16),
              const Text(
              ...
              ...stats['trust_level_distribution'].entries.map(
                (entry) => _buildStatRow(entry.key.toUpperCase(), entry.value),
              ),
            ],
          ),
        ),
